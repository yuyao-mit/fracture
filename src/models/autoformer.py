...
class PhaseFieldPredictor(nn.Module):
    def __init__(self):
        ...
        cfg = AutoformerConfig(
            ...
            context_length=self.T - 1,  # Use T-1 as context length
            prediction_length=self.pred_len,
            lags_sequence=[1],  # Simple lag sequence
            d_model=256,
            encoder_layers=3,
            decoder_layers=3,
            ...
            moving_average=25,  # Window size for moving average decomposition
            autocorrelation_factor=3,  # Factor for autocorrelation mechanism
        )
        self.autoformer = AutoformerForPrediction(cfg)
    ...
    def forward(self, x):
        ...
        # Encode spatial patches for each time step
        tokens_per_t, hw = [], None
        for t in range(T):
            tok, hw = self.enc(x[:, t])
            tokens_per_t.append(tok)
        
        # Stack tokens across time dimension
        zt = torch.stack(tokens_per_t, dim=1)  # Shape: [B, T, N, D]
        
        # Reshape for Autoformer processing
        seq = rearrange(zt, 'b t n d -> (b n) t d')  # [B*N, T, D]
        ...
        # Extract prediction from encoder output
        if hasattr(out, 'encoder_last_hidden_state') and out.encoder_last_hidden_state is not None:
            encoder_out = out.encoder_last_hidden_state  # Shape: [B*N, seq_len, hidden_size]
            
            # Take the last time step from encoder output
            if encoder_out.dim() == 3:
                last_hidden = encoder_out[:, -1, :]  # [BN, hidden_size]
                
                # Match dimensions
                if last_hidden.shape[1] == D:
                    y_pred = last_hidden
                elif last_hidden.shape[1] > D:
                    y_pred = last_hidden[:, :D]  # Truncate
                else:
                    # Pad with zeros
                    y_pred = torch.cat([
                        last_hidden, 
            ...
            # Use loc (location parameter) as backup
            loc = out.loc
            if loc.dim() == 1 and loc.shape[0] == BN:
                y_pred = loc.unsqueeze(-1).expand(-1, D)  # [BN, D]
            elif loc.dim() == 2:
            ...
            # Try last_hidden_state as another option
            decoder_out = out.last_hidden_state
            if decoder_out.dim() == 3:
                last_hidden = decoder_out[:, -1, :]  # [BN, hidden_size]
                if last_hidden.shape[1] >= D:
                ...
                raise ValueError(f"Unexpected last_hidden_state shape: {decoder_out.shape}")
                
        else:
            raise ValueError("No suitable prediction tensor found in Autoformer output")
        ...
        return y_frame.unsqueeze(1)  # Shape: [B, 1, C, H, W]
    
    def generate_prediction(self, x):
        """Alternative method using generate() for inference"""
        B, T, C, H, W = x.shape
        assert (T, C, H, W) == (self.T, self.C, self.H, self.W)
        ...
        try:
            gen_out = self.autoformer.generate(
                past_values=seq,
                past_time_features=past_tf,
                past_observed_mask=past_mask,
                future_time_features=future_tf,
            ...
            if hasattr(gen_out, 'sequences'):
                predictions = gen_out.sequences.mean(dim=1)  # Average over samples
                y_pred = predictions[:, -1, :]  # Take last prediction step
            else:
                raise ValueError("Generate output does not contain sequences")
                
        except Exception as e:
            print(f"Generate method failed: {e}, falling back to forward pass")
