...
class STGCN(nn.Module):
    def __init__(self, input_shape, output_shape):
        ...
        if self.Ko > 1:
            ...
            self.fc1 = nn.Linear(64, 64, bias=self.enable_bias)
            self.fc2 = nn.Linear(64, C_out, bias=self.enable_bias)
            self.relu = nn.ReLU()
            self.do = nn.Dropout(p=self.droprate)
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, self.C_out, self.T_out, self.H, self.W)
        x = x.permute(0, 2, 1, 3, 4)
